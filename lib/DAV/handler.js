        ...
        function afterCheck() {
            ...
            self.getHTTPHeaders(uri, function(err, httpHeaders) {
                ...
                if (!ignoreRangeHeader && nodeSize && range) {
                    ...
                    if (hasStream) {
                        var fileStream = node.getStream(start, end);
                        fileStream.on("error", function(err) {
                          if(self.httpResponse.headersSent) {
                            self.handleError(err);
                          } else {
                            self.httpResponse.end();
                            console.error("jsDAV GET error on creating file read stream", err);
                          }
                          return;
                        });

                        self.httpResponse.writeHead(206, httpHeaders);
                        fileStream.pipe(self.httpResponse);
                    }
                    else {
                    ...
                    if (!((since && lastModified === since) || (etag && oldEtag === etag))) {
                        ...
                        if (hasStream) {
                            ...
                            var fileStream = node.getStream(null, null);
                            fileStream.on("error", function(err) {
                              if(self.httpResponse.headersSent) {
                                self.handleError(err);
                              } else {
